...
const filterProducts = products.filter((product) => product.title.toLowerCase().includes(searchTerm.toLowerCase()));
    ...
    return (
        <div className="relative w-full mx-auto max-w-xs sm:max-w-lg md:max-w-full xl:max-w-screen-2xl mb-6">
            ...
            <input placeholder="Search for products ..." className="w-full max-w-xs sm:max-w-lg md:max-w-full xl:max-w-screen-2xl px-4 py-1 border rounded-full  border-oak-brown hover:brightness-105 duration-300 bg-light-beige font-medium tracking-wide" value={searchTerm} onChange={(event) => setSearchTerm(event.target.value.trim())} />
